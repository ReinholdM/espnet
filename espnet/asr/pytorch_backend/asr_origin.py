...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        # random choice 15% training data for mixup
        seed = np.random.RandomState(1)
        if seed.uniform(0, 1) > 0.15:
            self.mixup = False
        if self.is_mixup:
            x_ = _recursive_to(batch, self.device)
        loss_ = self.model(*x_).mean() / self.accum_grad
        loss = self.lam * loss + (1 - self.lam) * loss_
