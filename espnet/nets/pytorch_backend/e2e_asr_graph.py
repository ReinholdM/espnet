...
# Modified by lhmeng
from espnet.nets.alignme.avg import get_mid_len_v2
from espnet.nets.alignme.avg import avg_hs
from torch.nn.utils.rnn import pack_padded_sequence
from torch.nn.utils.rnn import pad_packed_sequence
from espnet.nets.pytorch_backend.rnn.enc_attention import att_enc_for
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        # Graph on encoder output
        frame_nums = []
        for i in range(xs_pad.shape[0]):
            frame_nums.append(len(list(filter(lambda x: x != 0, xs_pad[i, :, 0]))))
        total_frames = get_mid_len_v2(ys_pad, frame_nums)
        hs_avg_pad, avg_len = avg_hs(hs_pad, hlens, total_frames)
        a = None
        b = None
        #att_c, att_w = self.att_enc(hs_avg_pad, avg_len, a, b)
        att_idx = min(0, len(self.att_enc) - 1)
        self.att_enc[att_idx].reset()
        _, att_w=self.att_enc[att_idx](hs_avg_pad, avg_len, a, b)

        batch = hs_pad.size(0)
        h_length = hs_pad.size(1)
        for j, frame_num in enumerate(total_frames):
            frame_num.insert(0, 0)
            for s in range(len(frame_num)):
                hs_pad[j][frame_num[s]:frame_num[s + 1], :] *= att_w[j][s]
