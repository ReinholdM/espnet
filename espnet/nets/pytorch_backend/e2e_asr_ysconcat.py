...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.is_mixup:
            ...
            xs_index_pack = []
            for j in range(len(xs_pack)):
                xs_index_pack.append(xs_pack[index[j]])
            xs_pad_mix = torch.Tensor()
            for i in range(xs_pad.shape[0]):
                s = torch.cat([xs_pack[i, x_mid_len[i]:,:],xs_index_pack[i, x_index_mid_len[i],:]], dim=0).unsqueeze(0)
