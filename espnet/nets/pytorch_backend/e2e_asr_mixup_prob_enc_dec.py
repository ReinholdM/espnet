    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.mtlalpha == 0:
            ...
            #self.loss_ctc = self.ctc(hs_pad, hlens, ys_pad)
                self.loss_ctc = lambda_ * self.ctc(hs_pad, hlens, ys_pad) + (1 - lambda_) * self.ctc(hs_pad, hlens, ys_pad[index, :])
            else:
                self.loss_ctc = self.ctc(hs_pad, hlens, ys_pad)
        ...
        if self.mtlalpha == 1:
           ...
           # self.loss_att, acc, _ = self.dec(hs_pad, hlens, ys_pad)
                l1, acc_1, _ = self.dec(hs_pad, hlens, ys_pad)
                l2, acc_2, _ = self.dec(hs_pad, hlens, ys_pad[index, :]) 
                #self.loss_att, acc, _ = lambda_ * self.dec(hs_pad, hlens, ys_pad) + (1 - lambda_) * self.dec(hs_pad, hlens, ys_pad[index, :])
                self.loss_att = lambda_ * l1 + (1 - lambda_) * l2
                acc = lambda_ * acc_1 + (1 - lambda_) * acc_2
            else:
                self.loss_att, acc, _ = self.dec(hs_pad, hlens, ys_pad)
        self.acc = acc
