...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, dropout_rate,
        ...
        #self.norm1 = nn.BatchNorm1d(num_features=size, eps=1e-12, affine=True, track_running_stats=False)
        #self.norm2 = nn.BatchNorm1d(num_features=size, eps=1e-12, affine=True, track_running_stats=False)
    ...
    def forward(self, x, mask, cache=None):
        ...
        if self.normalize_before:
            x = self.norm2(x)
            #x = self.norm2(x.transpose(1, -1)).transpose(1, -1)
