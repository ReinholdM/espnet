...
# Modified by lhmeng
from espnet.nets.alignme.split import get_mid_len_v2
from espnet.nets.pytorch_backend.nets_utils import pad_list
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.is_mixup:
            ...
            for i in range(xs_pad.shape[0]):
                nz_num =len(list(filter(lambda x:x!=0, xs_pad[i, :, 0])))
                s = torch.cat((xs_pad[i,0:x_mid_len[i],:],xs_index_pack[i,x_index_mid_len[i] :nz_num,:]), dim=0)
                xs_pad_mix.append(s)
