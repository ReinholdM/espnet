...
# Modified by lhmeng
from espnet.nets.alignme.split_char import get_mid_len_v2
from espnet.nets.pytorch_backend.nets_utils import pad_list
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if max(frame_nums) > 600:
            self.is_mixup = False
        logging.info("mixup "+ str(self.is_mixup))
        if self.is_mixup:
            # Mixup inputs
            index = torch.randperm(xs_pad.shape[0]).cuda()
            #index = torch.randperm(xs_pad.shape[0])
            # mixup xs_pad, ys_pad
            #logging.info("xs_pad "+ str(xs_pad))
            bsz = xs_pad.shape[0]
            frame_nums = []
            for i in range(xs_pad.shape[0]):
                frame_nums.append(len(list(filter(lambda x:x!=0, xs_pad[i, :, 0]))))
            s = index.cpu().detach().numpy().tolist()
            logging.info("frame_nums "+str(frame_nums))
            frame_index_nums = []
            for i in range(len(frame_nums)):
                frame_index_nums.append(frame_nums[index[i]])
            logging.info("frame_index_nums "+str(frame_index_nums))
            x_mid_len,wrd_nums = get_mid_len_v2(ys_pad, frame_nums, self.cut_degree)
            logging.info("x_mid_len "+str(x_mid_len))
            flag = True
            if len(wrd_nums) == 0:
                flag = False
            if flag:
                #x_index_mid_len,wrd_index_nums = get_mid_len_v2(ys_pad[index, :], frame_index_nums, self.cut_degree)
                x_index_mid_len = []
                wrd_index_nums = []
                ...
                logging.info("wrd_nums"+str(wrd_nums))
                logging.info("wrd_index_nums"+str(wrd_index_nums))
                """
                x_mid_len = get_mid_len_v2(ys_pad)
                x_index_mid_len = get_mid_len_v2(ys_pad[index, :])
                logging.info("x_mid_len " + str(x_mid_len))
                """
            
                #xs_pack = [x[x!=0] for x in xs_pad]
                xs_index_pack = xs_pad[index,:,:]
                xs_pad_mix = []
                for i in range(xs_pad.shape[0]):
                    s = torch.cat((xs_pad[i,:x_mid_len[i],:],xs_index_pack[i,x_index_mid_len[i]:,:]), dim=0)
                    xs_pad_mix.append(s)
                    #xs_pad_mix = torch.stack((xs_pad_mix, s), dim=0)
                    #logging.info("s shape"+str(len))
                xs_pad = pad_list(xs_pad_mix, 0) 
                logging.info("xs_pad shape"+str(xs_pad.shape))
                """
                xs_pad_1 = xs_pad
                xs_index_pad_2 = xs_pad[index, :, :]
                for i in range(xs_pad.shape[0]):
                    logging.info("xs_pad_1[i, :, :] shape "+ str(xs_pad_1[i, :, :].shape))
                    logging.info("xs_pad[i, :x_index_mid_len[i], :] shape " + str(xs_pad[i, :x_index_mid_len[i], :].shape))
                    xs_pad_1[i, x_mid_len[i]:, :] = torch.zeros_like(xs_pad[i, x_mid_len[i]:, :])
                    xs_index_pad_2[i, :x_index_mid_len[i], :] = torch.zeros_like(xs_pad[index[i], :x_index_mid_len[i], :])
                #xs_pad_mix = torch.cat((xs_pad[:, :x_mid_len, :], xs_pad[index, x_index_mid_len:,:]))
                #xs_pad_mix = torch.cat((xs_pad_1, xs_index_pad_2), 1)
                xs_pad_mix = xs_pad_1 + xs_index_pad_2
            

                #xs_pad = xs_pad_mix
                logging.info("xs_pad_mix shape "+str(xs_pad_mix.shape))
                """
                ys = [y[y!=self.ignore_id] for y in ys_pad]
                ys_index = [y[y!=self.ignore_id] for y in ys_pad[index, :]]
                y_mid_len = []
                y_index_mid_len = []
                #results = [torch.cat([y[int(len(y)/2):], x[:int(len(x)/2)]], dim=0) for y, x in zip(ys, ys_index)]
                results = []
                cnt = 0
                for y,x in zip(ys, ys_index):
                    logging.info("cnt "+str(cnt))
                    ...
                    logging.info("space_id len "+str(len(space_id)))
                    logging.info("cut_nums "+str(cut_nums))
                ...
                ys_pad = ys_pad_mix
                logging.info("ys_pad_mix " + str(ys_pad_mix))
                """
                ys_pad_1 = ys_pad
                ys_index_pad_2 = ys_pad[index,:]
                for i in range(ys_pad.shape[0]):
                y_mid_len.append(int(len(ys_pad[i, :]) / 2))
                ys_pad_1[i,:int(len(ys_pad[i,:])/2)] = torch.zeros_like(ys_pad[i, :int(len(ys_pad[i,:])/2)])
                y_index_mid_len.append(int(len(ys_pad[index[i], :] / 2)))
                ys_index_pad_2[i,int(len(ys_pad[index[i],:])/2):] = torch.zeros_like(ys_pad[index[i], int(len(ys_pad[index[i],:])/2):])
                #ys_pad_mix = torch.cat((ys_pad[:, :y_mid_len], ys_pad[index, y_index_mid_len:]))
                ys_pad_mix = ys_pad_1 + ys_index_pad_2
                """
                logging.info("xs_pad[0] "+ str(xs_pad[0]))
                """
                # 0. Frontend
                if self.frontend is not None:
                    hs_pad_mix, hlens_mix, mask_mix = self.frontend(to_torch_tensor(xs_pad_mix), ilens)
                    hs_pad_mix, hlens_mix = self.feature_transform(hs_pad_mix, hlens_mix)
                else:
                    hs_pad_mix, hlens_mix = xs_pad_mix, ilens


                # 1. Encoder
                hs_pad_mix, hlens_mix, _ = self.enc(hs_pad_mix, hlens_mix)
                """

            logging.info("xs_pad shape[0]"+str(xs_pad.shape[0]))
            
 
            """
            mixup_samples = []
            for i in range(xs_pad.shape[0]):
               # random choice 15% training data for mixup-lhmeng
               seed = np.random.RandomState(1)
               if seed.uniform(0, 1) > 0.3:
                   self.is_mixup = False
               #if self.mixup and i != xs_pad.shape[0] - 1:
               #    xs_pad[i,:,:] = self.lam * xs_pad[i,:,:] + (1-self.lam) * xs_pad[i + 1,:,:]
               #    ys_pad[i,:] = self.lam * ys_pad[i, :] + (1-self.lam) * ys_pad[i + 1, :]
               if self.is_mixup and i != 0:
                   mixup_samples.append(i)
                   xs_pad[i,:,:] = self.lam * xs_pad[i,:,:] + (1-self.lam) * xs_pad[i - 1,:,:]
                   logging.info("mixup_samples "+str(i))
            """
        ...
        if self.mtlalpha == 0:
            ...
            if self.is_mixup:
                ...
                #self.loss_ctc_mix = self.ctc(hs_pad_mix, hlens_mix, ys_pad_mix)
                #self.loss_ctc = 0.5 * self.loss_ctc + 0.5 * self.loss_ctc_mix
            else:
                self.loss_ctc = self.ctc(hs_pad, hlens, ys_pad)
        logging.info("ctc_loss "+" "+str(self.loss_ctc))

        """
        logging.info("hlens size" + str(len(hlens)))
        for j in mixup_samples:
            self.loss_ctc += ((self.lam - 1) * self.ctc(hs_pad[j, :, :].unsqueeze(0), hlens[j], ys_pad[j]))/ hs_pad.size(0) + \
            ((1 - self.lam) * self.ctc(hs_pad[j, :, :].unsqueeze(0), hlens[j], ys_pad[j-1,:]))/ hs_pad.size(0)
        """

        # Reduce the dimension of xs_pad, ilens,hs_pad, hlens from 2Bsz to Bsz
